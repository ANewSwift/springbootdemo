...
//        okHttpUtil.sendPostRequest(TEST_URL, headers, "{\"code\":\"TGHNSZ0058\",\"defaultDcCode\":\"HBBJ03\",\"financialCenterCode\":\"00.09\",\"isBread\":1,\"virtualDcCode\":\"HBBJ92\",\"distributionDistance\":66.8,\"transferStrategy\":\"DEFAULT\"}");
        String url = "http://localhost:8081/springboot-demo/user/create";
        OkHttpUtil okHttpUtil = new OkHttpUtil();
...
//            urls.add(url);
            ...
            User user = User.builder().name("test"+i).age(i).operator("test").build();
            okHttpUtil.sendPostRequest(url,null, JsonUtil.convertJsonString(user));
        }
//        okHttpUtil.batchSendGetRequest(urls);
    }
...
//                .headers(Headers.of(headers))
        ...
        call.enqueue(new Callback() {
            ...
            public void onResponse(Call call, Response response) throws IOException {
                ...
                //如果希望返回的是字符串
                final String responseData = response.body().string();
                System.out.println(LocalDateTime.now() + "success: " + response.isSuccessful() + ", " +responseData);
            }
        });
    }

    private void batchSendGetRequest(List<String> urls) {
        //创建okHttpClient对象
        OkHttpClient okHttpClient = new OkHttpClient();
        urls.forEach(url -> {
            //创建request,首先要有一个url
            ...
            call.enqueue(new Callback() {
                ...
                public void onResponse(Call call, Response response) throws IOException {
                    ...
                    //如果希望返回的是字符串
                    final String responseData = response.body().string();
                    System.out.println(LocalDateTime.now() + "success: " + responseData);
//                //如果希望返回的是二进制字节数组
